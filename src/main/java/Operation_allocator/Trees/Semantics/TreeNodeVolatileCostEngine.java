...
public class TreeNodeVolatileCostEngine {
    ...
    public static <T extends Operation> CostMetric simulateEncryptionAndComputeCost(TreeNode<T> tn, List<AttributeConstraint> la, Provider endp) {
        ...
        if (tn.isLeaf()) {
            ...
            //verify produced output relation profile compliance
            List<Attribute> newInSet = tempres.getThird();
            if (!TreeNodeSemantics.simulateOperationOutputRelProf(tn, newInSet, endp))
                return null;
            ...
            //verify produced output relation profile compliance
            List<Attribute> newInSet = tempres.getThird();
            if (!TreeNodeSemantics.simulateOperationOutputRelProf(tn, newInSet, endp) && !endp.selfDescription().equals(home.selfDescription()))
                return null;
        }
