...
/**
 * Union class does not automatically projects output attributes, it has to be specified what is projected
 * The same for homogeneous set
 * Remember that even constrained set is required to be fixed, it would be useless to apply union to tuples
 * wrapped with randomized symmetric encryption
 * Homogeneous set must be the whole attribute set
 */
public final class Union extends Operation implements Serializable {
    ...
    public Union() {
        super.familyname = "Union";
        projectedSet = new LinkedList<>();
        homogeneousSet = new LinkedList<>();
    }

    public Union(List<Attribute> pset, List<Attribute> fset) {
        super.familyname = "Union";
        projectedSet = pset;
        homogeneousSet = fset;
    }
