...
/**
 * Intersection class does not automatically projects output attributes, it has to be specified what is projected
 * The same for homogeneous set
 * Remember that even constrained set is required to be fixed, it would be useless to apply intersection to tuples
 * wrapped with randomized symmetric encryption
 * Homogeneous set must be the whole attribute set
 */
public final class Intersection extends Operation implements Serializable {
    ...
    public Intersection() {
        super.familyname = "Intersection";
        projectedSet = new LinkedList<>();
        homogeneousSet = new LinkedList<>();
    }

    public Intersection(List<Attribute> pset, List<Attribute> fset) {
        super.familyname = "Intersection";
        projectedSet = pset;
        homogeneousSet = fset;
    }
